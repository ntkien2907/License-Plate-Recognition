...
if YOLO_VERSION == 7:
    import torch
    model = torch.hub.load(YOLO_CONFIG, 'custom', YOLO_WEIGHT)

def object_detection_yolov7(img_path):
    img = cv2.imread(img_path)
    results = model(img)
    boxes = results.pandas().xyxy
    n_coords = []
    for box in boxes:
        xmin, ymin, xmax, ymax = box.iloc[0][:4]
...
def OCR(img_path, filename):
    if YOLO_VERSION == 7:
        n_coords = object_detection_yolov7(img_path)
    else:
